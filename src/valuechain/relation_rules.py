...
EXTRACTOR_VERSION = "rules-0.4.0-sec-exhibits-recall"
...
RELATION_PATTERNS: list[tuple[str, str, str, float]] = [
    ...
    (
        ...
        r"purchase agreement|master services agreement|services agreement|procurement agreement)\b",
        "supplier(s)",
        0.66,
    ),
    (
    ...
    def extract(self, passage: Passage) -> list[RelationEvidence]:
        ...
        for relation_type, pattern, object_hint, base_confidence in RELATION_PATTERNS:
            if passage.section.startswith("exhibit_21") and relation_type != "subsidiary_or_control":
                continue
            if not re.search(pattern, lowered) and not (
            ...
            if modality == "current_fact":
                ...
                confidence -= 0.05
            max_objects = 8 if relation_type == "subsidiary_or_control" or passage.section.startswith("exhibit_21") else 5
